...
void execute_A(){
	//funcao que adiciona uma mensagem, guarda a actividade de um utilizador, o tamanho da mensagem, e conta o numero de mensgens totais
...
void execute_U(){
	...
	int selected_user, counter_user_messages, counter_aux, index, i=0, user_text[MAX_MSG];
	scanf("%d",&selected_user);
	//guarda o index das mensagens de um utilizador do vector global que contem todas as mensagens num vector
	for (counter_aux=0; counter_aux<counter; counter_aux++){
	...
	//no vetor user_text que contem os index das mensagens de un dado utilizdor da print de todas elas
...
void execute_S(){
  //ordena as mensagens por ordem alfabetica, utilizando o insertion sort, no entanto nao se ordena a estrutura mensagem, 
  //mas sim os indices das mensagens num array (sortList) ordenando-os dependendo do valor obtido na funcao less
  int i,j; 
  for (i = 1; i < counter; i++) { 
	int v = sortList[i]; 
	j = i-1;
   ...
   //imprime as mensagens percorrendo a sortList, onde as mensagens ja estao ordenadas
   for(i = 0; i < counter; i++)
...
void execute_C(){
	...
	//Usa a strtok de modo a conseguir comparar a palavra selecionada com todas as palavras de todas as strings
...
int user_activity(int sel_user){
	...
	for (counter_aux=0; counter_aux<counter; counter_aux++){
		if (sel_user==message[counter_aux].user)
			sel_user_msg++;
	}
	return sel_user_msg;
}
